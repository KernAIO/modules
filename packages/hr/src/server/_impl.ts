...
import { MODULE_ID, hrContract, hrEvents } from '../contract.js'
...
export function implement_(kernel: Kernel) {
  ...
  return os.router({
    notes: {
      list: scoped.notes.list.use(requires('hr.note.view')).handler(({ input }) =>
